...
export const RUNC_DOWNLOAD_SCRIPT: TCustomValueItems = {
  data: {
    value: dedent`
      #!/bin/bash
      set -Eeuo pipefail
      ...
      COMPONENT_VERSION="$\{COMPONENT_VERSION:-${COMPONENTS_VERSION.runc.value}}"
      REPOSITORY="$\{REPOSITORY:-https://github.com/opencontainers/runc/releases/download}"
      PATH_BIN="$\{REPOSITORY}/${DOWNLOAD_VERSION.runc.templateUrlBin}"
