...
import { CONTAINERD_DOWNLOAD_SCRIPT } from '@site/i18n/en/docusaurus-plugin-content-docs/current/tech-docs/kubernetes/components/containerd/download-script.sh'
  ...
  <TabItem value='Bash'>
      ...
      <CodeBlock language="bash">
        {dedent`
          ...
          COMPONENT_VERSION="${COMPONENTS_VERSION.containerd.value}"
          REPOSITORY="${DOWNLOAD_VERSION.containerd.baseUrl}"
          EOF
        `}
      </CodeBlock>
      ...
      <CodeBlock language="bash">
        {dedent`
          ...
          ${CONTAINERD_DOWNLOAD_SCRIPT.data.value}
          EOF
        `}
      </CodeBlock>
      ...
      <h4>Starting the download service</h4>
      <CodeBlock language="bash">
        {dedent`
