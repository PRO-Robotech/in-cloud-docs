...
<Tabs groupId="etcd-node">
  <TabItem value='Master-1'>
    ...
    5. Remove old files from the directory <code>{ETCD_ARGS.dataDir.value}</code>
    ...
    :::note
    Note that the local `etcdctl` file is used and the snapshot is also located on the master in the directory <code>{ETCD_ARGS.workDir.value}</code>.
