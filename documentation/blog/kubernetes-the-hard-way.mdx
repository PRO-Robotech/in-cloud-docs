...
description: 'Пошаговое ручное развертывание Kubernetes-кластера без kubeadm: сертификаты, static pod-ы, kubeconfig и полная настройка управляющего контура.'
date: 2025-03-23
...
<p>
  ...
  <strong>внутри самой системы</strong>. Это требует четкого порядка действий:
</p>
...
<ul>
  ...
  <li>
    ⏱ <strong>Нельзя запускать все одновременно</strong><br />
    Параллельный запуск приводит к нежелательным результатам.<br />
    <em>Пример:</em> Планировщик ждет API, а API — загрузку и инициализацию данных.
  </li>
  <li>
  ...
  <li>
    ...
    Конфиги, сертификаты, адреса — все подготавливается вручную.<br />
...
Без строго определенной последовательности такая система не заработает.
Именно поэтому существуют инструменты и утилиты, например, <code>kubeadm</code>, которые устраняют проблему
«курицы и яйца» и задают корректный порядок развертывания.
...
<ul>
  ...
  <li>
    <a href="#28-добавление-worker-нод">28. Добавление Worker-нод</a>
  </li>
</ul>
...
<p>Развертывание Kubernetes вручную требует дополнительных усилий. Однако, данный подход обладает рядом преимуществ:</p>

<ul>
  <li>Обеспечивает углубленное понимание архитектуры и внутренней логики работы компонентов Kubernetes.</li>
...
<p>
  Ниже приведен список компонентов, необходимых для ручного развертывания кластера. Для обеспечения совместимости все
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Распределенное хранилище типа "ключ–значение" для хранения конфигурации и состояния кластера.</td>
      </tr>
      <tr>
...
<p>
  На этом этапе задается базовая архитектура кластера, включая его сетевую схему, управляющие узлы и основные параметры.
</p>
...
> Этот раздел посвящен проверке состояния компонентов кластера после запуска kubelet. Здесь описываются команды для контроля загрузки образов, запуска контейнеров и успешной инициализации кластерных ресурсов. Это позволяет убедиться, что кластер стартовал корректно, до перехода к следующим этапам.
...
### 28. Добавление Worker-нод

> Настройка управляющего контура завершена. Кластер работает, но пока не имеет рабочих узлов для выполнения нагрузок.
> Следующий шаг — расширение кластера Worker-нодами.
>
> Подробное пошаговое руководство по добавлению Worker-нод доступно в отдельной статье:
...
Если подводить итог: эта статья писалась около **четырех месяцев**.
Каждый скрипт был вылизан вручную (при участии `chatGPT`) и протестирован в реальных условиях.
Без шуток — за все это время я поднял **более 400 кластеров**.
