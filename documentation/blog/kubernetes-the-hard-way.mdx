...
<p>
  В данной статье рассматривается пошаговая ручная настройка кластера — подход <strong>«The Hard Way»</strong>. Для
  каждой фазы <code>kubeadm</code> будет показано, как достичь аналогичного результата без использования
...
> В данном разделе рассматриваются правила использования сертификатов в кластере Kubernetes: какие компоненты задействуют сертификаты, кем они подписываются и как осуществляется проверка подлинности.
  ...
  <FancyboxImage src="/img/certificates/certificates-base.drawio.svg" />
