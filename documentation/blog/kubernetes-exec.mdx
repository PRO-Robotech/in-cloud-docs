...
<h3> Kubernetes pods/exec <span style={{ color: '#1cc5ac' }}>#</span> </h3>

<p style={{ fontStyle: 'italic', fontWeight: 600 }}>
  Разбираем переход на WebSocket и его влияние на RBAC.
</p>

<div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: '30px', marginTop: '1.5rem' }}>
  <div style={{ flex: '1 1 58%', minWidth: '280px', maxWidth: '620px' }}>
    <p>
      ...
      Но вот вам сходу задача: как вы считаете, насколько безопасна вот такая роль в Kubernetes, которую можно выдать любому пользователю, предположив абсурдную ситуацию, что ресурс Secret в кластере не используется?
    </p>
  </div>
...
<p>
  Если вы ответили «безопасная», то вы не одиноки — и вы предоставили бы доступ к <code>pods/exec</code> любому пользователю в кластере.
</p>
...
К сожалениея нельзя знать все на свете, но можно стараться минимизировать риски.
