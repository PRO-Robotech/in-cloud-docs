...
description: 'Настройка политик аудита Kubernetes: уровни логирования, фильтрация событий, подавление системного шума и примеры конфигураций для продакшен-кластеров.'
date: 2025-03-23
...
Сами события можно направить в файл, на удаленный webhook или в систему логирования.

## Политика аудита

Политика аудита — это YAML-файл, описывающий набор правил, определяющих, какие события нужно логировать и с какой степенью детализации. Каждое правило состоит из условий (фильтров) и уровня логирования `level`, который задает объем сохраняемой информации.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Запрос получен API-сервером, но еще не обработан</td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>Запрос полностью завершен</td>
    </tr>
    <tr>
      <td>
        <code>Panic</code>
...
- Используйте `omitStages` для снижения объема логов
