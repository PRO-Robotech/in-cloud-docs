...
<CrushedNodeFunctionsK8s.Provider>
  ...
  5. Cоберите все возможные пререквизиты для подключения к ETCD и K8s кластеру.

  <Tabs groupId="etcd-status">
    <TabItem value='ETCD кластер работает'>
      6. Если удается подключиться к ETCD кластеру, тогда создайте резервную копию
      и выполните восстановление кластера с дальнейшим подключением
      остальных узлов ETCD кластера.
    </TabItem>
    <TabItem value='ETCD кластер не работает'>
      6. Если подключение к ETCD кластеру невозможно, сделайте резервную копию файлов БД.
      ...
      восстановлению с одного узла для `Master-2` и `Master-3`.
    </TabItem>
  </Tabs>
