...
<ThreeNodeFunctionsK8s.Provider>
  ...
  11. Повторите этапы сбора пререквизитов и подключения к кластеру ETCD.

  12. При помощи команд `member list` и `endpoint status` описанных выше проверьте состояние кластера.
