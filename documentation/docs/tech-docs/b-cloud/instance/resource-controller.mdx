...
- **Управление развертыванием**: позволяет настраивать базовые конфигурации для автоматического развертывания клиентских компонентов.
...
6. Действия процесса реконсиляции при удалении:
   - В ответ на событие удаления, процесс реконсиляции должен удалять все объекты, связанные с удаляемым ресурсом Project.
...
<Tabs>
    ...
    <TabItem value='ResourceQuota'>
        При создании ресурса `Instance` автоматически генерируется ресурс `ResourceQuota`, который впоследствии будет отвечать за распределение квот в проекте.
        <CodeBlock language="yaml">
          {dedent`
            ---
            apiVersion: v1
    ...
    <TabItem value='ApplicationSET'>
        При создании ресурса `Instance` автоматически генерируется ресурс `ApplicationSET`, который впоследствии будет отвечать за развертывание клиентских компонентов в кластере.
        <CodeBlock language="yaml">
          {dedent`
            ---
    ...
    <TabItem value='InstanceLegacy'>
      При создании ресурса `Instance` автоматически генерируется ресурс `InstanceLegacy`, который впоследствии будет отвечать за интеграцию с внешними системами.
      <CodeBlock language="yaml">
        {dedent`
              ---
              apiVersion: v1
    ...
    <TabItem value='Secret'>
      ...
      TODO @Колбин - проверить и дописать!.
      <CodeBlock language="yaml">
        {dedent`
                ---
                apiVersion: v1
