...
```bash
kubectl get pods -l app.kubernetes.io/name=metrics-server --namespace bcloud-monitoring-metrics
```
...
```bash
kubectl get endpoints/metrics-server --namespace bcloud-monitoring-metrics  -o json

```
Выберите нужный под, к примеру `metrics-server-6c65f6987f-v6djh` и запустите:

```bash
kubectl port-forward pod/metrics-server-6c65f6987f-v6djh 10250:10250 --namespace bcloud-monitoring-metrics
```
