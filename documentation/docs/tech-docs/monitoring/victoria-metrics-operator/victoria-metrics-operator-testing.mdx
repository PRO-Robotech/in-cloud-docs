...
```bash
kubectl get pods -l app.kubernetes.io/name=victoria-metrics-operator --namespace bcloud-vmcluster
```
...
```bash
kubectl get endpoints/victoria-metrics-operator --namespace bcloud-vmcluster  -o json

```
Выберите нужный под, к примеру `victoria-metrics-operator-6c65f6987f-v6djh` и запустите:

```bash
kubectl port-forward pod/victoria-metrics-operator-6c65f6987f-v6djh 10250:10250 --namespace bcloud-vmcluster
```
