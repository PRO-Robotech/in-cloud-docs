...
`Node Exporter` устанавливается как обычное Kubernetes-приложение, описанное с помощью Helm Chart, которое можно установить либо из b-cloud manifests либо через helm.
...
```bash
helm install prometheus-node-exporter prom/prometheus-node-exporter --namespace bcloud-monitoring-nodes --create-namespace -f values.yaml
```

Команда создаст релиз с именем `node-exporter` и развернет `Node Exporter` на вашем кластере Kubernetes в неймспейсе `bcloud-monitoring-nodes`.
...
После этого выполните команду установки с указанием вашего конфигурационного файла:
    ...
    <CodeBlock language="bash">
      {dedent`helmfile -e $\{PROMETHEUS_NODE_EXPORTER_VERSION} -l bcloud-collections=prometheus-node-exporter bcloud-monitoring-stack=client | kubectl apply -f - `}
    </CodeBlock>
