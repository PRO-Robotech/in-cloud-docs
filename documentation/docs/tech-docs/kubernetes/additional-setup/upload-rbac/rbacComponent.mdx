...
<Tabs groupId="install-type">
  ...
  <details className="custom-gray-block">
    ...
    > Этот токен — bootstrap-токен, и он нужен для того, чтобы разрешить новой ноде безопасно присоединиться к Kubernetes-кластеру через kubeadm join, пока у нее еще нет собственных сертификатов и доверенного kubeconfig-а.
