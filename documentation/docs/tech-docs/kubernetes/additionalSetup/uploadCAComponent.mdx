...
<Tabs groupId="install-type">
  ...
  <TabItem value='HardWay'>
    ...
    <CodeBlock language="bash">
      {dedent`
        cat <<EOF > ${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/openssl/encrypt.py
        #!/usr/bin/env python3
    ...
    <CodeBlock language="bash">
      {dedent`
        cat <<EOF > ${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/openssl/upload-certs.sh
        #!/bin/bash
        set -euo pipefail

        CERT_PATH="${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/pki"
