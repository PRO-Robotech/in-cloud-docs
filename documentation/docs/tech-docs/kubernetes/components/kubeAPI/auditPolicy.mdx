...
> Политика аудита определяет, какие запросы к API Server логируются и с какой детализацией. Файл загружается при старте kube-apiserver через `--audit-policy-file`; изменения требуют перезапуска.
