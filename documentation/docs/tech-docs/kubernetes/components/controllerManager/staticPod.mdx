...
> `--cluster-cidr` задаёт диапазон IP-адресов для Pod-сети, используемый контроллером IPAM; `--service-account-private-key-file` указывает приватный ключ для подписи токенов ServiceAccount; `--cloud-provider` (при наличии) подключает интеграцию с облачным провайдером для управления нодами и балансировщиками.
