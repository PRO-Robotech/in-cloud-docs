...
<CodeBlock language="bash">
  {dedent`
    kubectl config set-cluster kubernetes \\
      ...
      --kubeconfig=${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/kubelet.conf

    kubectl config set-credentials system:node:$\{FULL_HOST_NAME} \\
      --client-certificate=${CERTIFICATES.kubeletClient.crtPath} \\
      --client-key=${CERTIFICATES.kubeletClient.keyPath} \\
      --embed-certs=true \\
      --kubeconfig=${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/kubelet.conf
...
<CodeBlock language="bash">
  {dedent`
    ${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/openssl/cert-report.sh ${CERTIFICATES.kubeletCurrentClient.crtPath}
  `}
</CodeBlock>
