...
> Приватный ключ, указываемый в `kube-apiserver` и `kube-controller-manager`, используется для **подписания токенов** этих аккаунтов. Это обеспечивает **безопасное и проверяемое** взаимодействие между сервисами и дает возможность **гранулярного контроля доступа**.

> В этом разделе создается или подключается ключ, с помощью которого Kubernetes подписывает токены `ServiceAccount`.
