...
:::warning
Обратите внимание, что для инициализации кластера используется заранее подготовленный конфигурационный файл ```kubeadm``` с предуказанным токеном для будущих мастер-нод, что упрощает установку. В промышленной эксплуатации рекомендуется генерировать уникальный токен для каждого узла с ограниченным временем жизни для повышения безопасности.
:::

Данная команда впитывает в себя разделы:
- [Проверка готовности узла](./components/componentsReady.mdx).
- [Генерация ЦА](./certificates/centerAuthority/init.mdx).
- [Генерация Сертификатов/Kubeconfigs](./certificates/components/initCerts.mdx).
...
- [Загрузка конфигурационных файлов в kubernetes](./additionalSetup/uploadConfigsInit.mdx).
- [Загрузка корневых сертификатов в kubernetes](./additionalSetup/uploadCA.mdx).
- [Маркировка узлов](./additionalSetup/markingInit.mdx).
- [Создание токенов для подключения и ролевой модели](./additionalSetup/rbac.mdx).


<CodeBlock language="bash">
    {dedent`
        kubeadm init --config=${CUSTOM_VALUE.kuberneteBaseFolderPath.value}/kubeadm.conf --upload-certs
    `}
</CodeBlock>
